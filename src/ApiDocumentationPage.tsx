...
const API_BASE = String(import.meta.env.VITE_SGP_PARTNER_API_BASE || "/api/v1").replace(/\/$/, "");

const assistantCurl = `curl -X POST "${API_BASE}/assistant/query" \\
  -H "X-API-Key: $SGP_KLP_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "query": "What approaches have supported community-led reef restoration?",
    "source": "all"
  }'`;
...
const embedServer = `const response = await fetch("${API_BASE}/embed/sessions", {
  method: "POST",
  headers: {
    "X-API-Key": process.env.SGP_KLP_API_KEY,
...
const { sessionId, tier, expiresAt } = await response.json();`;
    ...
    <header className="api-docs-hero">
      <div className="content-width">
        ...
        <div className="api-docs-actions">
          <a className="button button--primary" href={`${API_BASE}/openapi.json`} download><Download size={17} /> {tx("download")}</a>
        ...
        <div className="api-release-strip" aria-label={tx("interfaceStatus")}>
          ...
          <div><Status kind="live">{tx("temporaryRuntime")}</Status><span>{tx("temporaryRuntimeBody")}</span></div>
          <div><Status kind="planned">{tx("productionGate")}</Status><span>{tx("productionGateBody")}</span></div>
        </div>
      </div>
    </header>
      ...
      <article className="api-docs-content">
        <section id="overview" className="api-section">
          ...
          <div className="api-capability-grid">
            <div><Bot /><h3>{tx("assistant")}</h3><p>{tx("assistantCapability")}</p><Status kind="live">{tx("temporaryRuntime")}</Status></div>
            <div><Search /><h3>{tx("search")}</h3><p>{tx("searchCapability")}</p><Status kind="live">{tx("temporaryIndex")}</Status></div>
            <div><Database /><h3>{tx("data")}</h3><p>{tx("dataCapability")}</p><Status kind="live">{tx("temporaryRuntime")}</Status></div>
            <div><Radio /><h3>{tx("exchange")}</h3><p>{tx("exchangeCapability")}</p><Status kind="planned">{tx("productionGate")}</Status></div>
          </div>
        ...
        <section id="quickstart" className="api-section">
          ...
          <div className="api-note"><strong>{tx("temporaryBase")}</strong><p>{tx("temporaryBaseBody", { base: API_BASE })}</p></div>
        </section>

        <section id="authentication" className="api-section">
          <p className="eyebrow">{tx("navAuthentication")}</p><h2>{tx("authTitle")}</h2>
          <p>{tx("temporaryBaseBody", { base: API_BASE })}</p><p>{tx("authBody")}</p>
        ...
        <section id="assistant" className="api-section">
          ...
          <div className="api-endpoint"><span>POST</span><code>/assistant/query</code><Status kind="live">{tx("temporaryRuntime")}</Status></div>
          <p>{tx("assistantApiBody")}</p>
          <CodeBlock title={tx("responseJson")} code={`{
  "tier": "authenticated",
  "text": "Community-led reef restoration examples...",
  "documents": [{
    "id": "res_8f2a",
    ...
    "source": "SGP archive",
    "relevance": 12,
    "dataset": "innovation_library"
  }],
  "ideas": ["Compare these examples"]
}`} {...codeLabels} />
        ...
        <section id="search" className="api-section">
          ...
          <div className="api-endpoint"><span>GET</span><code>/resources/search</code><Status kind="live">{tx("temporaryRuntime")}</Status></div>
          <p>{tx("resourceSearchBody")}</p>
          <CodeBlock title={tx("resourceSearch")} code={resourceSearch} {...codeLabels} />
          <div className="api-endpoint"><span>POST</span><code>/documents/search</code><Status kind="live">{tx("temporaryIndex")}</Status></div>
          <p>{tx("productionDocumentGateBody")}</p>
          <CodeBlock title={tx("permissionedSearch")} code={documentSearch} {...codeLabels} />
          <div className="api-note api-note--warning"><strong>{tx("productionDocumentGate")}</strong><p>{tx("productionDocumentGateBody")}</p></div>
        </section>

        <section id="datasets" className="api-section">
          <p className="eyebrow">{tx("dataAccess")}</p><h2>{tx("datasetsTitle")}</h2>
          <div className="api-endpoint"><span>GET</span><code>/datasets</code><Status kind="live">{tx("temporaryRuntime")}</Status></div>
